...
void main() {
  testWidgets('prevents a duplicate submission and sends the ISO date only', (
    WidgetTester tester,
  ) async {
    ...
    expect(find.byKey(const Key('session-form-topic')), findsNothing);
    expect(gateway.invocations.single.payload.containsKey('topic'), isFalse);

    completer.complete(const <String, Object?>{'id': 'ses1', 'revision': 1});
    await tester.pumpAndSettle();
  });

  testWidgets('backend failure keeps the date', (
    WidgetTester tester,
  ) async {
    final FakeAcademicGateway gateway = FakeAcademicGateway()
